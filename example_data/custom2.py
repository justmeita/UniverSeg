...
class CUSTOM2Dataset(Dataset):
    dataset: Literal["Kidney"]
    split: Literal["support", "test"]
    label: Optional[Literal["kidney", "tumor", "background"]] = None
    support_frac: float = 0.7

    def __post_init__(self):
        root = require_download_custom()
        path = root / {"Kidney" : "Kidney"}[self.dataset]
        T = torch.from_numpy
        self._data = [(T(x)[None], T(y)) for x, y in load_folder(path)]
        if self.label is not None:
            self._ilabel = {"tumor":2, "kidney": 1, "background": 0}[self.label]
