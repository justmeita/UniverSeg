...
@dataclass
class CUSTOM3Dataset(Dataset):
    dataset: Literal["Pugs"]
    split: Literal["support", "test"]
    label: Optional[Literal["pug","not defined", "background"]] = None
    support_frac: float = 0.7

    def __post_init__(self):
        root = require_download_custom()
        path = root / {"Pugs" : "Pugs"}[self.dataset]
        T = torch.from_numpy
        self._data = [(T(x)[None], T(y)) for x, y in load_folder(path)]
        if self.label is not None:
            self._ilabel = {"pug":2, "not defined": 1, "background": 0}[self.label]
